...
def _fill_color(ws, color, from_row, from_col, to_row = None, to_col = None):
    ...
    # Set color of cells from cell('from_row', 'from_col') to cell('to_row', 'to_col') as 'color'.
    # 'color' = 19 : Ivory
    #           36 : Light Yellow
    #           38 : Rose
    ...
    if to_col is None:
        to_col = from_col
    ws.Range(ws.Cells(from_row, from_col), ws.Cells(to_row, to_col)).Interior.ColorIndex = color
...
def _merge_for_link(ws, cur_row, col, num_SH):
    ...
    # This function is supposed to be called by _print_column_name() and _print_row_item().
...
def _print_column_name(ws, metric, column_name):
    ...
    # This function is supposed to be called from print_overall() and _print_Metric().
...
def _print_row_item(ws, row_name, metric, list_1D, SH_per_link = None, display_min = False):
    # Input
    # > 'ws'            : Excel worksheet.
    # > 'row_name'      : str.
    # > 'metric'        : enum 'Metric'.
    ...
    # This function is supposed to be called from _print_Metric() and print_overall().
    ...
    def _display_minmax_value(list_of_tuple, number, find_max):
        ...
        # > 'find_max'      : boolean.
        #
        # Cells with the 'number' largest/smallest values will be colored by _fill_color().

        # If the number of elements in 'list_of_tuple' is less then 'number', color all cells.
        ...
        thsh = list_of_tuple[number - 1][0]     # thsh = list_of_tuple[number-1].item
    ...
    if metric == Metric.Link:   # Modify list_1D in case of link metric.
        ...
        for index in range(len(SH_per_link) - 1, -1, -1):
            for _ in range(SH_per_link[index][1] - 1):
                target_list.insert(index + 1, 99999999 if display_min else -99999999)  # Insert dummy value.
    else:   # lane, TT, Node
        target_list = list_1D
    ...
    _display_minmax_value(to_find_max, 3, False if (metric == Metric.TT) or display_min else True)
...
def print_explanation(ws):
    ...
    _print_text(ws, "* Delay : Total delay divided by total travel time of all vehicles in this link segment [%]")
    _print_text(ws, "* Density : Vehicle density [/km]")
    _print_text(ws, "* Emissions CO : Quantity of carbon monoxide [g]")
    _print_text(ws, "* Emissions VOC : Quantity of volatile organic compounds [g]")
    _print_text(ws, "* LOS : Level of service (A ~ F).")
    _print_text(ws,
                "* OccupRate : Share of time [0% ~ 100%] of the last simulation step, in which at least one data collection point of this data collection measurement was occupied.")
    _print_text(ws,
                "* QueueStop : The number of queue stops per meter. A queue stop counts when a vehicle that is directly upstream or within the queue length falls below the speed of the Begin attribute defined for the queue condition. [/m]")
    _print_text(ws, "* Speed : Average speed of vehicles passing through the section [km/h]")
    _print_text(ws, "*")
...
def print_overall(ws, lanes_with_SH, SH_per_link, No_Node, DelayRel_overall, Density_overall, AvgSpeed_overall, QStop_overall, OccupRate_overall, EmissionCO, EmissionVOC):
    # Input
    # > 'ws'            : Excel worksheet.
    ...
    _print_row_item(ws, "Speed",        Metric.Link, AvgSpeed_overall,  SH_per_link, display_min = True)
...
def print_hour(ws, lanes_with_SH, SH_per_link, Link_TT, No_Node, VehNum_hour, QStop_hour, OccupRate_hour, AvgSpeed_hour, LOS_hour, EmissionCO_hour, EmissionVOC_hour):
    # Input
    # > 'ws'            : Excel worksheet.
    # > 'lanes_with_SH' : 1D list of (int(LinkNo), int(LaneNo), double(PosSH), double(LinkLen)).
    # > 'SH_per_link'   : 1D list of (int(LinkNo), int(NumSH)).
    # > 'Link_TT'       : 1D list of (str(StartLink), str(EndLink)) or empty list.
    # > 'No_Node'       : 1D list of int(NodeNo) or empty list.
    ...
    def _print_Metric(metric_name, metric, column_name, list_2D):
        ...
        ws.Cells(row - 1, 2).Value = str(hour) + "~END"  # Last one has to be overwritten.
    ...
    _print_Metric("* The Number of Vehicles",   Metric.Lane, lanes_with_SH, VehNum_hour)
    ...
    if No_Node:
        ...
        _print_Metric("* Emissions VOC",    Metric.Node, No_Node, EmissionVOC_hour)
